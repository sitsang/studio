...
public class c {
  ...
  Socket io(Socket s) throws IOException {
      /*if (s instanceof SSLSocket) {
        try {
            ((SSLSocket)s).startHandshake();
        } catch (SSLHandshakeException ex) {
            System.err.println("SSL connection failed, revert to regular connection");
            s = new Socket(s.getInetAddress().getHostName(),s.getPort());
        }          
      }*/
...
*/
  ...
  private void startReader() {
    ...
    public void reconnect(boolean retry) throws IOException,K4Exception {
        ...
        //int localPort = ssh.init(host,port);
        //s = new Socket("localhost", localPort);
        ssh.init(host);
        s = new Socket(host, port);
        //System.out.println("Connected to " + localPort);
        ...
        int readByte = inputStream.read(bytes,0,1);
        if (1 != readByte)
            if(retry)
                reconnect(false);
            else
