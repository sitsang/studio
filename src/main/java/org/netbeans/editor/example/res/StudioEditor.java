...
import org.netbeans.editor.example.Editor;
...
public class StudioEditor extends JFrame
{
        ...
        public void actionPerformed(ActionEvent evt)
        {
            ...
            if (!handleOpenRecent(src))
            {
                if (src == openItem)
                {
                    ...
                    VFSJFileChooser.RETURN_TYPE returnVal = fileChooser.showOpenDialog(StudioEditor.this);
    ...
    private boolean saveAs(Component comp)
    {
        ...
        if (fileChooser.showSaveDialog(this) == VFSJFileChooser.RETURN_TYPE.APPROVE)
    ...
    private void readSettings() throws MissingResourceException
    {
        ...
        try {
            ...
            UserAuthenticator auth = new StaticUserAuthenticator(null, "kdbrun", "");
            DefaultFileSystemConfigBuilder.getInstance().setUserAuthenticator(new FileSystemOptions(), auth);
            FileObject file = manager.resolveFile("sftp://sddsvrny003.scglobal.ad.scotiacapital.com/");
            fileChooser = new VFSJFileChooser(file);
        } catch (Exception ex) {
