...
public class Config
{
    ...
    public Font getFont()
    {
        ...
        if( p != null)
        {
            ...
            int s= (int) Math.round(14 * Toolkit.getDefaultToolkit().getScreenResolution() / 96.0);
            if( size != null)
    ...
    private void init()
    {
        ...
        passwordEncryptor.setPassword("STUDIO_FOR_KDB");
    ...
    public void removeServerByName(String serverName)
    {
        ...
        for( int i= 0; i < servers.length; i ++)
        {
            if( ! serverName.equals( servers[i].getName()))
            {
                l.add( servers[i]);
                server = servers[i];
            }
    ...
    private void setServerDetails( Server server)
    {
        ...
        p.setProperty( "server."+name + "." + "password", "" + password);
